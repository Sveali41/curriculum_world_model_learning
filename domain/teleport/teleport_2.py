...
from minigrid.core.world_object import Door, Goal, Key, Wall  # type: ignore
    ...
    def step(
        ...
        # Move forward
            ...
            if fwd_cell is not None and fwd_cell.type == "lava":
                terminated = True
            if fwd_cell is not None and fwd_cell.type == "teleporter":
                if fwd_cell.is_active:
...
class Teleport5by5(TeleportBaseEnv):
    def __init__(
        self,
        size=7,
        agent_start_pos=(1, 5),
    ...
    def _gen_grid(self, width, height):
        ...
        self.grid.vert_wall(width // 2, 0, length=3)
        ...
        destination_teleporter_l = Teleporter(active=False)
        destination_teleporter_r = Teleporter(active=False)
        self.put_obj(source_teleporter, 3, 4)
        self.put_obj(destination_teleporter_l, 2, 2)
        self.put_obj(destination_teleporter_r, 4, 2)

        # Place a goal square in the top-left corner
        # and the top-right corner
        self.put_obj(Goal(), 1, 1)
        self.put_obj(Goal(), 5, 1)
